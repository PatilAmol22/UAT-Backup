...
import getTranslationMap from '@salesforce/apex/GTMPathFinder.getTranslationMap';
...
export default class GtmCategoryAllocation extends LightningElement {
    ...
    @track productAllocations = [];
     mapTranslation = []; //Added by Priyanka(SKI) on 26/10/2022
    hasRendered = false;
    ...
    connectedCallback(){
        ...
        // Added for Product Category Translation Change by Priyanka(SKI) on 28-09-2022
        getTranslationMap().then(Translation=>{
            console.log('mapTranslation2', Translation);
          this.mapTranslation = Translation;
            console.log('mapTranslation3', this.mapTranslation);
        })
    ...
    mapCustomerCategory(productAllocation,customerid){
        ...
        productAllocation.forEach(ele=>{
            ...
            let obj = {'pId':ele.Product_Category__r.Id,'pName':this.mapTranslation[ele.Product_Category__r.Id],'GTMDetail':ele.Id,'allocation':ele.Product_Category_Allocation__c,isSubmitted__c:ele.isSubmitted__c,helpText:helptext}; // Added Tranlated Product Category by Priyank(SKI) on 28-09-2022 
            arr.push(obj)
