...
import getCropBreakupData from '@salesforce/apex/LiquidationSWALController.getCropBreakupData';
import createCropLiquidation from '@salesforce/apex/LiquidationSWALController.createCropLiquidation';
...
export default class LiquidationSWALCropCmp extends LightningElement {
    ...
    connectedCallback(){
        ...
        .then(result => {
            if(result != null && result != undefined){
			    ...
			    for(var i=0; i < res.liqSWALCropList.length ; i++){
                    this.cropNameData.push(res.liqSWALCropList[i].Name);
                }					
				var crpLiqTotal = 0;
                var balanceValue = 0;
                var crpLiqData = [];
                ...
                var prevMnthLiqYTD;
                //Creating Table  data
                for(var ti = 0; ti < res.swalProdLiqList.length; ti ++){
					//Crop liq List for each Product
                    crpLiqData = [];
                    crpNme = [];
                    crpLiqTotal = 0;
                    balanceValue = 0;
                    for(var ci = 0; ci < res.swalCropLiqList.length; ci ++){
                       if(res.swalCropLiqList[ci].Liquidation__r.Product__r.Product_Code__c == res.swalProdLiqList[ti].Product__r.Product_Code__c){
                            if(!isCropSub){
                                var crpJson = {"crpLiqId" : '',
                                                "liqId" : res.swalProdLiqList[ti].Id,
                                                "crpName" : res.swalCropLiqList[ci].Crop__r.Name,
                                                "crpId" : res.swalCropLiqList[ci].Crop__r.Id,
                                                "prdCode" : res.swalCropLiqList[ci].Liquidation__r.Product__r.Product_Code__c,
                                                "curLiqValue" : res.swalCropLiqList[ci].Liquidation_Value__c,
                                                "prevLiqValue" : res.swalCropLiqList[ci].Liquidation_Value__c,
                                                "prodYTD" : res.swalProdLiqList[ti].Liquidation_MTD_SWAL__c,
                                                "prevMonthLiqYTD" : res.swalCropLiqList[ci].Liquidation_MTD_SWAL__c,
                                                "sortOrder" : res.swalCropLiqList[ci].Crop__r.Crop_Order__c,																   
                                                "crpIndex" : ti
                                            };
                                crpNme.push(res.swalCropLiqList[ci].Crop__r.Name); 
                                prevMnthLiqYTD = res.swalCropLiqList[ci].Liquidation__r.Liquidation_MTD_SWAL__c;           
                            }else{
                                var crpJson = {"crpLiqId" : res.swalCropLiqList[ci].Id,
                                                "liqId" : res.swalProdLiqList[ti].Id,
                                                "crpName" : res.swalCropLiqList[ci].Crop__r.Name,
                                                "crpId" : res.swalCropLiqList[ci].Crop__r.Id,
                                                "prdCode" : res.swalCropLiqList[ci].Liquidation__r.Product__r.Product_Code__c,
                                                "curLiqValue" : res.swalCropLiqList[ci].Liquidation_Value__c,
                                                "prevLiqValue" : res.swalCropLiqList[ci].Prev_Month_Crop_Liquidation_Volume__c,
												"prodYTD" : res.swalCropLiqList[ci].Liquidation_YTD_Trade__c,														   
                                                "prevMonthLiqYTD" : res.swalCropLiqList[ci].Liquidation_YTD_Prev_Month__c,
                                                "sortOrder" : res.swalCropLiqList[ci].Crop__r.Crop_Order__c,
                                                "crpIndex" : ti
                                            };
                                crpNme.push(res.swalCropLiqList[ci].Crop__r.Name); 
                            }
                            crpLiqTotal = parseFloat(crpLiqTotal) + parseFloat(res.swalCropLiqList[ci].Liquidation_Value__c);
                            crpLiqData.push(crpJson);
                        }
                    }

                    //Handling new Crop Logic
                    if(crpNme.length != res.liqSWALCropList.length){
                        res.liqSWALCropList.forEach(function(crp){
                            if(!crpNme.includes(crp.Name)){
                                ...
                                crpLiqData.push({
                                    ...
                                    "crpName" : crp.Name,
                                    "crpId" : crp.Id,
                                    "prdCode" : res.swalProdLiqList[ti].Product__r.Product_Code__c,
                                    "curLiqValue" : 0,
                                    "prevLiqValue" : 0,
									"prodYTD" : res.swalProdLiqList[ti].Liquidation_MTD_SWAL__c,														   
                    ...
                    balanceValue = parseFloat(res.swalProdLiqList[ti].Liquidation_MTD_SWAL__c) - parseFloat(crpLiqTotal.toFixed(2));

                    //Final table value
                    var liqJson = {"liqId" : res.swalProdLiqList[ti].Id,
                                    "prdName" : res.swalProdLiqList[ti].Product__r.Name,
                                    "prdCode" : res.swalProdLiqList[ti].Product__r.Product_Code__c,
                                    "prdYTD" : res.swalProdLiqList[ti].Liquidation_MTD_SWAL__c,
                                    "rowIndex" : ti,
