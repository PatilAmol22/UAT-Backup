...
import getEuropeOrg from "@salesforce/apex/Grz_InventoryEurope.getEuropeOrg";
// import getInventories from "@salesforce/apex/GetEuropeInventoryData.getInventories";
export default class Grz_inventoryEuropeParent extends LightningElement {
...
if (result.data) {
    ...
    console.log("orgData==" + JSON.stringify(result.data));
    console.log("orgData==" +result.data[0].countrySpecific);
this.salesOrgShow = result.data[0].countrySpecific;
    result.data.forEach((ele) => {
    if (ele.Sales_Org_Code__c == "2921") {
        this.countryAlias = "HU";
    } else if (ele.Sales_Org_Code__c == "2081") {
        this.countryAlias = "RO";
    }
     else if (ele.Sales_Org_Code__c == "2471") {
        this.countryAlias = "BG";
    }
    let obj = {
...
let elements = this.template.querySelectorAll("c-grz_data-list-europe");
console.log("data====" + JSON.stringify(elements));
...
CommonChildCall(event){
    console.log('this.searchDepot',this.searchDepot);
    console.log('this.isChecked',this.isChecked);
    console.log('this.clickedOrgList',this.clickedOrgList);
    console.log('this.selectedValue',this.selectedValue);
    console.log('this.searchData',this.searchData);
let elements = this.template.querySelectorAll("c-grz_inventory-europe-child");
console.log("data====CommonChildCall" + JSON.stringify(elements));
for (let i = 0; i < elements.length; i++) {
