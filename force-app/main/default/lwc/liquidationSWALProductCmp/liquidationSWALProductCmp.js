...
import getProductLiquidationData from '@salesforce/apex/LiquidationSWALController.getProductLiquidationData';
import createProductLiquidation from '@salesforce/apex/LiquidationSWALController.createProductLiquidation';
...
export default class LiquidationSWALProductCmp extends LightningElement {
	//Custom Label
    label = {
    ...
    connectedCallback(){
        ...
        if(this.liquidationuserrole == 'FMM'){
            this.isFMMHasRole = this.checkFMMRole(this.liquidationterritory);
        }else if(this.liquidationuserrole == 'ZML'){
            this.isZMLHasRole = this.checkZMLRole(this.liquidationterritory);
        }		
        
        if(this.liquidationuserrole == 'TM' || this.liquidationuserrole == 'RM'){
        ...
        }else if(this.liquidationuserrole == 'FMM' && this.isFMMHasRole){
            this.getProductTableData(this.liquidationterritory, this.liquidationfiscalyear, this.liquidationmonth,this.liquidationuserrole, this.isTMAvailable);
        }else if(this.liquidationuserrole == 'ZML' && this.isZMLHasRole){
            this.getProductTableData(this.liquidationterritory, this.liquidationfiscalyear, this.liquidationmonth,this.liquidationuserrole, this.isTMAvailable);
        }else{
    ...
    //Check FMM Role for territory
    checkFMMRole(terrCode){
        const isZsmRoleJson = JSON.parse(this.isfmmrolemap);
        return isZsmRoleJson[terrCode];
    }

    //Check ZML Role for territory
    checkZMLRole(terrCode){
        const isZsmRoleJson = JSON.parse(this.iszmlrolemap);
        return isZsmRoleJson[terrCode];
    }
        ...
        .then(result => {
            if(result != null && result != undefined){
                if(result == noLAPErrorMessageLabel || result == liqNotSubErrorMessageLabel){
                    ...
                    const selectedEvent = new CustomEvent('errormessage'); 
                    this.dispatchEvent(selectedEvent);

                }else{
                    const selectedEvent = new CustomEvent('productdata'); 
                    this.dispatchEvent(selectedEvent);
                    var res = JSON.parse(result);
                    this.allowAllByPass = res.byProdPassValid;   //By pass validation
                    this.liqCurStatus = res.liqStatus;
                    for(var pi = 0; pi < res.liqSWALProductList.length; pi++){
                        this.productTableData.push(res.liqSWALProductList[pi]);
                        if(!res.liqSWALProductList[pi].isDisableOI){
    ...
    handleSubmitClick(){
        ...
        if((this.liquidationuserrole == 'RM' || this.liquidationuserrole == 'FMM' || this.liquidationuserrole == 'ZML') && this.liqCurStatus == 'Pending for Approval'){
    ...
    handleCropClick(event){
        const selectedEvent = new CustomEvent('crop'); 
        this.dispatchEvent(selectedEvent);
