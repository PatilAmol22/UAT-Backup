...
import ActiveSalesOrgForMaterialRequisition from '@salesforce/label/c.ActiveSalesOrgForMaterialRequisition';// Added by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
import getSalesOrgbyEmail  from '@salesforce/apex/MaterialReqMassApproval.getSalesOrgbyEmail';// Added by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
...
salesOrgCode = ''// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
...
@wire(getSalesOrgbyEmail,{hoEmail:'$HO_EMAIL'}) getSalesOrg({error,data}){
  if(error){
    console.log(error);
  }
  if(data){
    console.log('SalesOrg',data);
    this.salesOrgCode = data;
  }
}
// Edited by Sandeep Vishwakarma - SKI 27-01-2023
connectedCallback(){
  this.HO_EMAIL = location.search.split("=")[1].split("&")[0];
  getSalesOrgbyEmail({hoEmail:this.HO_EMAIL}).then(salesorg=>{
    this.salesOrgCode = salesorg;
    this.hasRenderquery=false;
    console.log(' salesorg ',salesorg);
  }).catch(err=>{
    console.log('err SalesOrg',err);
  })
}
renderedCallback(){
  this.verifyCheckbox();
  // this.salesOrgCode = JSON.stringify(this.salesOrgCode).replace('[','(').replace(']',')').replaceAll('"',"'");
  console.log('this.salesOrgCode ',this.salesOrgCode);// Edited by Sandeep Vishwakarma - SKI 27-01-2023
...
checkSession(){
  isSessionActive({hoEmail:this.HO_EMAIL,otp:this.OTP}).then(isActive=>{
    if(!isActive){
      ...
      // https://upl--hotfix.sandbox.my.salesforce-sites.com/
      location.replace("https://upl--uat.sandbox.my.salesforce-sites.com/requisitionapprover");// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
    }
  })
}
...
// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
        let query = `select Id,Name,Depot__c,Depot__r.Name,Depot__r.HO_Commercial__c,Territory__c,Territory__r.Name,Territory_Manager__c,Territory_Manager__r.Name,Approval_Submission_Date_Time__c,(Select Id,Name,Product__c,Product__r.Name,Dose_Acre_GM_ML_L_Per_Acre__c,Demo_Size_Acer__c,Number_of_Demo__c,Demo_Sample_Qty_GM_ML__c,Demo_material_value__c from Free_Sampling_Products__r),(select Id,Name,Target_Crop__c,Target_Crop__r.Name,Target_Pest__c,Target_Pest__r.Name,recordType.Name from Free_Sampling_Crop_Pests__r) from Free_Sample_Management__c where Sales_Org__r.Sales_Org_Code__c='${this.salesOrgCode}' and Territory__c!=null and Status__c='Pending' and Sub_Status__c='Pending from HO Commercial' and Depot__r.HO_Commercial__c='${this.HO_EMAIL}' order by CreatedDate ASC `;
        if(recordName){
