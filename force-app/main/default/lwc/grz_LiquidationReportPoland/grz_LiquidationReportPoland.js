...
import getPOGLiquidationReportData from '@salesforce/apex/Grz_POGLiquidationReportController.getPOGLiquidationReportData';
...
import POGReport from '@salesforce/label/c.Grz_POGReport';
import MultiSelectPicklistLabel from '@salesforce/label/c.Grz_MultiSelectPicklistLabel';
import SKUCodeFilterLabel from '@salesforce/label/c.Grz_SKUCodeFilterLabel';
...
export default class Grz_LiquidationReportPoland extends LightningElement {
    ...
    labels = {
        ...
        SKUCodeFilterLabel: SKUCodeFilterLabel,
    ...
    handleSelectSKUOptionList(event){
        ...
        /*if ( (this.distributor != '' || this.distributorName==All) && this.year != '' && this.quarter != ''){
            this.getLiquidation( this.distributor,this.distributorName, this.year, this.quarter,this.selectedSkuList1);
        }*/
       ...
       // var selectedCols = [];
        for(var i in this.selectedValueList){
            ...
            if(this.selectedValueList[i].selected){
                console.log('this.selectedValueList[i]',this.selectedValueList[i]);
        ...
        console.log('selectedCols-->>',this.selectedCols);

        /*let tempselectedColumn = [];
        for(var j in tepOptions){
            console.log('tepOptions j : ',tepOptions[j].value);
            tempselectedColumn.push({value:tepOptions[j].value,selected:false});
            for(var i in this.selectedValueList){
                console.log('value : ',this.selectedValueList[i]);
                if(this.selectedValueList[i]==tepOptions[j].value){
   ...
   renderedCallback(){
    ...
    getUserCountry({ error, data }) {
        this.spinner = true;
        console.log('getUserCountry data ',data);
        if (data) {
    ...
    handleOnfocus(){
        ...
        //if(this.year == new Date().getFullYear() || this.year == new Date().getFullYear()-1){    // Updated for INC0413721 GRZ(Dheeraj Sharma) 18-11-2022
            if(this.year == new Date().getFullYear()){    // Updated for INC0413721 GRZ(Dheeraj Sharma) 18-11-2022
    ...
    TypePicklistValues_city({ data, error }) {
        ...
        if (data) {
            ...
            data.values.forEach(element => {
                ...
                if (element.value.split('-')[0] == new Date().getFullYear()-2) {
                ...
                if (element.value.split('-')[0] == new Date().getFullYear() || element.value.split('-')[0] == new Date().getFullYear()-1) {
                    ...
                    //if(this.Q1.includes(new Date().getMonth()) || this.Q3.includes(new Date().getMonth()) || this.Q4.includes(new Date().getMonth())){
                        /*this.year = element.value.split('-')[0];
                        console.log('this.year : '+this.year);
                        this.selected_year = element.value;
                        console.log('this.selected_year : ',this.selected_year);*/
    ...
    @track columns = [
        ...
        { Id: 5, label: this.labels.OpeningInventoryPLN, fieldName: 'OpeningInventoryPLN', type: 'text', initialWidth: 180, editable: this.is_editable, largewidth: true },
        { Id: 6, label: this.labels.ytd_sales, fieldName: 'YTD_sales', type: 'text', initialWidth: 280 },
        { Id: 7, label: this.labels.YTD_SalesPLN, fieldName: 'YTD_SalesPLN', type: 'text', initialWidth: 280 },
        { Id: 8, label: this.labels.Total_Available_Stock, fieldName: 'Total_Available_Stock', type: 'text', initialWidth: 200, sortable: true },
        { Id: 9, label: this.labels.TotalAvailableStockPLN, fieldName: 'TotalAvailableStockPLN', type: 'text', initialWidth: 200},
        { Id: 10, label: this.labels.Distributor_Inventory, fieldName: 'Distributor_Inventory', type: 'text', initialWidth: 125, editable: this.is_editable },
        { Id: 11, label: this.labels.DistributorInventoryPLN, fieldName: 'DistributorInventoryPLN', type: 'text', initialWidth: 125, editable: this.is_editable },
        //{ Id: 8, label: this.labels.Plan_YTD_Quarter, fieldName: 'Plan_for_the_Quarter', type: 'text', initialWidth: 150 },
        { Id: 12, label: this.labels.Liquidation_YTD_Selected_Quarter, fieldName: 'Liquidation_YTD', type: 'text', initialWidth: 280 },
        { Id: 13, label: this.labels.Liquidation_YTDSelectedQuarterPLN, fieldName: 'Liquidation_YTD_PLN', type: 'text', initialWidth: 280 },
        { Id: 14, label: this.labels.Liquidation_YTD_Selected_Quarter_Percent, fieldName: 'Liquidation_percentage_YTD', type: 'text', initialWidth: 280, sortable: true },
    ...
    getLiquidation(t_distributor,t_year, t_quarter, selectedSkuList,ischeckDisInventory) {
        ...
        if (t_year != '' && t_quarter != '') {
            ...
            getPOGLiquidationReportData({ distributor: t_distributor, year: t_year, quarter: t_quarter,skuList: selectedSkuList,excludeDisInventory: ischeckDisInventory}).then(data => {
                ...
                data.forEach(element => {
                    temp.push({
                       ...
                       // 'Liquidation_YTD_per_plan_YTD': isFinite(element.Liquidation_YTD__c / element.Plan_for_the_month__c) ? Number(((element.Liquidation_YTD__c / element.Plan_for_the_month__c) * 100).toFixed(2)).toLocaleString(this.number_saperator_code) + ' %' : '0 %',
                        ...
                        'key_di': element.SKU_Code + '-di',
                        'key_ri': element.SKU_Code + '-ri',
                        'key_pfnm': element.SKU_Code + '-pfnm'
                    })
                ...
                /*this.pogPDFUrl = '/apex/Grz_LiquidationReportPolandPDF?distributor='+ t_distributor+'&distributorName='+t_distributorName
                +'&year='+t_year+'&quarter='+t_quarter+'&skuList='+selectedSkuList;*/

                // this.pogPDFUrl = '/apex/Grz_LiquidationReportPolandPDF?distributor='+ t_distributor
                // +'&year='+t_year+'&quarter='+t_quarter+'&skuList='+selectedSkuList+'&excludezero='+this.ischecked;
                // console.log('this.pogPDFUrl : '+this.pogPDFUrl);

                // this.pogXLSUrl = '/apex/Grz_LiquidationReportPolandXLS?distributor='+ t_distributor
                // +'&year='+t_year+'&quarter='+t_quarter+'&skuList='+selectedSkuList+'&excludezero='+this.ischecked;
    ...
    downloadPDF() {
        ...
        // (this.selectedAccountList1,this.year, this.quarter,this.selectedSkuList1,this.ischecked);

        this.pogPDFUrl = '/apex/Grz_LiquidationReportPolandPDF?distributor='+ this.selectedAccountList1
        +'&year='+this.year+'&quarter='+this.quarter+'&skuList='+this.selectedSkuList1+'&excludezero='+this.ischecked+'&selectedColumn='+ this.SelectedColumnValue+'&search='+this.search_str;   /* Start GRZ(Butesh Singla) : APPS-1395 PO And Delivery Date :17-07-2022 */
        console.log('this.pogPDFUrl : '+this.pogPDFUrl);
    ...
    downloadXLS() {
        this.pogXLSUrl = '/apex/Grz_LiquidationReportPolandXLS?distributor='+ this.selectedAccountList1
                +'&year='+this.year+'&quarter='+this.quarter+'&skuList='+this.selectedSkuList1+'&excludezero='+this.ischecked+'&selectedColumn='+ this.SelectedColumnValue+'&search='+this.search_str;   /* Start GRZ(Butesh Singla) : APPS-1395 PO And Delivery Date :17-07-2022 */
        this.spinner = true;
        const downloadLink = document.createElement("a");
