...
import getPOGLiquidationReportDataCustomer from '@salesforce/apex/Grz_POGLiquidationReportController.getPOGLiquidationReportDataCustomer';
...
import POGReport from '@salesforce/label/c.Grz_POGReportCustomer';
...
import SelectBrand from '@salesforce/label/c.Grz_SelectBrand';
...
export default class Grz_reportByCustomerTable extends LightningElement {
    ...
    @track dependentSKUOptions=[];
    @track isParentPicklistSelected = false;
    ...
    @track options = [
        ...
        { label: this.labels.Distributor, value: 'Brand' },
    ...
    handleSelectBrandOptionList(event){

        let checkboxes = this.template.querySelector('[data-id="checkbox"]');
        checkboxes.checked = true;
        this.ischecked = true;

        this.selectedBrandValueList = JSON.parse(JSON.stringify(event.detail));
        console.log('this.selectedBrandValueList : ',this.selectedBrandValueList);

        var selectedBrandList = [];
        for(var i in this.selectedBrandValueList){
            //console.log('value : ',this.selectedBrandValueList[i]);
            selectedBrandList.push(this.selectedBrandValueList[i]);
        }
        this.selectedBrandList1 = selectedBrandList;
        console.log('this.selectedBrandList1 : ',this.selectedBrandList1);
        
        this.isParentPicklistSelected = true;
        this.selectedSkuList1 = [];
        if ( this.year != '' && this.quarter != ''){
            this.getLiquidation( this.selectedAccountList1, this.year, this.quarter,this.selectedSkuList1,this.selectedBrandList1,this.ischecked);
        }
    ...
    handleSelectSKUOptionList(event){
        ...
        this.isParentPicklistSelected = false;
   ...
   renderedCallback(){
    ...
    @track columns = [
        { Id: 1, label: this.labels.Distributor, fieldName: 'Brand', type: 'text', initialWidth: 100, sortable: true },
    ...
    getLiquidation(t_distributor,t_year, t_quarter, selectedSkuList,selectedBrandList,ischeckDisInventory) {
        this.spinner = true;
        this.liquidation_data = [];
        ...
        if (t_year != '' && t_quarter != '') {
            ...
            getPOGLiquidationReportDataCustomer({ distributor: t_distributor, year: t_year, quarter: t_quarter,skuList: selectedSkuList,brandList:selectedBrandList,excludeDisInventory: ischeckDisInventory}).then(data => {

                console.log('Liquidation data : ', data);
                let temp = [];
                let temp_for_submit = [];
                var customerData = data.POGWrapDataList;
                var selectedsku = data.POGSkuDepnedentList;
                console.log('selectedsku : ', selectedsku);
                this.dependentSKUOptions = [];

                if(this.isParentPicklistSelected){
                    selectedsku.forEach(element => {
                        if(element.SKU_Description){
                            this.dependentSKUOptions = [...this.dependentSKUOptions, { label: element.SKU_Description, value: element.Id }];
                        }
                    });
                    console.log('Liquidation selectedsku : ', this.dependentSKUOptions);
    
                    const objChild = this.template.querySelector('c-grz_multi-select-sku-picklist');
                    objChild.setDependentSkuDecList(this.dependentSKUOptions);
                }
                ...
                customerData.forEach(element => {
                    temp.push({
                        ...
                        'Liquidation_percentage_YTD': element.Liquidation_YTD_per_plan_YTD !== undefined ? element.Liquidation_YTD_per_plan_YTD + ' %' : '0 %',
                ...
                // this.pogPDFUrl = '/apex/Grz_LiquidationCustomerPolandPDF?distributor='+ t_distributor
                // +'&year='+t_year+'&quarter='+t_quarter+'&skuList='+selectedSkuList+'&brandList='+selectedBrandList+'&excludezero='+this.ischecked;
                // console.log('this.pogPDFUrl : '+this.pogPDFUrl);

                // this.pogXLSUrl = '/apex/Grz_LiquidationCustomerPolandXLS?distributor='+ t_distributor
                // +'&year='+t_year+'&quarter='+t_quarter+'&skuList='+selectedSkuList+'&brandList='+selectedBrandList+'&excludezero='+this.ischecked;
    ...
    downloadPDF() {
        ...
        downloadLink.click();*/
        this.pogPDFUrl = '/apex/Grz_LiquidationCustomerPolandPDF?distributor='+ this.selectedAccountList1
        +'&year='+this.year+'&quarter='+this.quarter+'&skuList='+this.selectedSkuList1+'&brandList='+this.selectedBrandList1+'&excludezero='+this.ischecked+'&selectedColumn='+ this.SelectedColumnValue+'&search='+this.search_str;   /* Start GRZ(Butesh Singla) : APPS-1395 PO And Delivery Date :17-07-2022 */
    ...
    downloadXLS() {
        this.pogXLSUrl = '/apex/Grz_LiquidationCustomerPolandXLS?distributor='+ this.selectedAccountList1
        +'&year='+this.year+'&quarter='+this.quarter+'&skuList='+this.selectedSkuList1+'&brandList='+this.selectedBrandList1+'&excludezero='+this.ischecked+'&selectedColumn='+ this.SelectedColumnValue+'&search='+this.search_str;   /* Start GRZ(Butesh Singla) : APPS-1395 PO And Delivery Date :17-07-2022 */
        this.spinner = true; 
        const downloadLink = document.createElement("a");
