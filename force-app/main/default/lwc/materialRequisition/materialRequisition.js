...
import getAllProfiles from '@salesforce/apex/SampleMaterialRequisition.getAllProfiles';// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
import SWALProducts from '@salesforce/label/c.SWALProducts'; //RITM0541619 -  enable Sample Requisition tab for SWAL users with given products by Ritesh
...
export default class MaterialRequisition extends NavigationMixin(LightningElement) {
    ...
    @track crop_filter = '';
    @track pest_filter = '';
    @track territory_filter = `Sales_Org_Code__c = '${this.salesOrgCode}' and TerritoryManager__c=null limit 20`;// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
    ...
    @track dummyValue=''; // use for chaining of wire Apis // Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
    profiles = {// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
        'territory_manager':'',
        'FMM_Users':'',
        'regional_and_zonal_manager':'',
        'Channel_Deployment_Manager_SWAL':'',
        'Ho_commercial':''
    }
    @wire(getAllProfiles) allProfiles({error,data}){// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
        if(data){
            console.log('All Profiles  ',data);
            let tempProfiles = {
                'territory_manager':data[0]?.territory_Manager__c,
                'FMM_Users':data[0]?.fmmUser__c,
                'regional_and_zonal_manager':data[0]?.regional_Manager__c.split(';'),
                'Channel_Deployment_Manager_SWAL':data[0]?.change_Deployment_Manager__c,
                'Ho_commercial':data[0]?.ho_Marketing__c
            }
            this.profiles = tempProfiles;
            this.hasRendered.Freesampling = false;
            this.dummyValue = JSON.stringify(tempProfiles);
        }
        if(error){
            console.log('All Profiles Error ',error);
        }
    }
    ...
    salesOrgCode({error,data}){
        if(data){
            ...
            // RITM0541619 - enable Sample Requisition tab with change in Products for SWAL
            if(this.salesOrgCode == '1410'){
                this.product_filter = `Sales_Org_Code__c='${this.salesOrgCode}' and isActive=true order by Name ASC limit 15`;
                this.crop_filter = `Sales_Org_Code__c='1410' order by Name ASC limit 15`; //1000 AF
            }else{
                this.product_filter = `Sales_Org_Code__c='${this.salesOrgCode}' and isActive=true AND Product_Code__c IN (` +  SWALProducts + `) order by Name ASC`;
                this.crop_filter = `Sales_Org_Code__c='1210' order by Name ASC limit 15`; //SWAL
            }
            //this.crop_filter = `Sales_Org_Code__c='1410' order by Name ASC limit 15`; //1000
    ...
    @wire(getProfile,{dummyValue:'$dummyValue'})// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
    getLoggedInProfile({error,data}){
        ...
        if(data){
            ...
            if(this.profile_name==this.profiles.FMM_Users || this.profiles.regional_and_zonal_manager?.includes(this.profile_name)){
    ...
    getUser({error,data}){
        if(data){
            ...
            if(this.loggedinuser){// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
                this.territory_filter = `Sales_Org_Code__c = '${this.salesOrgCode}' and (FMM_User_Id__c='${this.loggedinuser}' OR (Regional_Manager_Id__c='${this.loggedinuser}' and FMM_User_Id__c=null) OR (Zonal_Marketing_Manager_1_Id__c='${this.loggedinuser}' AND Regional_Manager_Id__c=null and FMM_User_Id__c=null)) AND (TerritoryManager__c=null)`;
                console.log('territory_filter ',this.territory_filter);
            }
        }
    }

    @wire(getTerritory,{dummyValue:'$dummyValue'})
    getTerritory({error,data}){// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
        if(data){
            ...
            if(data.length>0 && this.freesamplemanagementid==''){ // Create
                ...
                console.log(' this.profiles.territory_manager ',this.profiles.territory_manager);
                if(this.profile_name!=this.profiles.territory_manager){ // back to list view
                    this.TerritoryNotFound = true;
                    setTimeout(() => {// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
                        this.isModalOpen = true;
                    }, 200);
                    console.log('Territory Manager has assigned ');
                }else{// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
    ...
    externalUserButtonMatrix(){
        // Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
        console.log('UserButton profileName ',this.profile_name,' status',this.freesampleObj.Status__c,' loggedinuser ',this.loggedinuser,'All Profiles ',this.profiles);
        ...
        console.log('condition 249',this.profiles.regional_and_zonal_manager?.includes(this.profile_name));
        // Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
        console.log('this.freesampleObj.Office_Manager__c ',this.freesampleObj.Office_Manager__c);
        ...
        if(this.freesampleObj.Status__c==this.status.Draft || this.freesampleObj.Status__c==this.status.reject){
            ...
            if((this.profile_name==this.profiles.territory_manager || this.profiles.regional_and_zonal_manager?.includes(this.profile_name) || this.profile_name==this.profiles.FMM_Users) && (this.freesampleObj.OwnerId==this.loggedinuser || this.freesampleObj.OwnerId == undefined)){
        ...
        if(this.freesampleObj.Status__c==this.status.Pending && this.freesampleObj.Sub_Status__c==this.sub_status.Pending_for_Approval_1 && this.freesampleObj.Office_Manager__c){// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
            this.is_TM = true;
            this.buttonmatrixobj.addbtn = true;
            this.disablefield(true,true,true,true,true,true);
            console.log('Test 218 ',this.profiles.regional_and_zonal_manager?.includes(this.profile_name) || this.profile_name == this.profiles.FMM_Users || this.profile_name==this.profiles.Channel_Deployment_Manager_SWAL);
            if(this.profiles.regional_and_zonal_manager?.includes(this.profile_name) || this.profile_name == this.profiles.FMM_Users || this.profile_name==this.profiles.Channel_Deployment_Manager_SWAL){
        ...
        if(this.freesampleObj.Status__c==this.status.Pending && (this.freesampleObj.Sub_Status__c==this.sub_status.Pending_for_Approval_2 || (this.freesampleObj.Sub_Status__c==this.sub_status.Pending_for_Approval_1 && !this.freesampleObj.Office_Manager__c))){// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
            this.is_TM = true;
            this.buttonmatrixobj.addbtn = true;
            this.disablefield(true,true,true,true,true,true);
            if(this.profiles.regional_and_zonal_manager?.includes(this.profile_name)  || this.profile_name == this.profiles.FMM_Users || this.profile_name==this.profiles.Channel_Deployment_Manager_SWAL){
    ...
    getFreeSampleManagement(data){
        ...
        this.territory.Name = data.Territory__c!=undefined?data.Territory__r.Name:'';
            if(this.territory.Id){
               this.getDepotByTerritory();// Edited by Sandeep Vishwakarma - SKI 27-01-2023 AF Material Requisition
            }
        }
        ...
        if(this.profile_name==this.profiles.territory_manager || this.profile_name==this.profiles.FMM_Users || this.profiles.regional_and_zonal_manager?.includes(this.profile_name)){
            getManagers().then(data=>{
                this.managersFound = data?true:false;
                console.log('Managers 1',JSON.parse(JSON.stringify(data)));
                if(this.profile_name==this.profiles.territory_manager){
                    this.freesampleObj.Territory__c = data.Id!=undefined?data.Id:'';
                }
                console.log('For  Sales Org ',this.salesOrgCode);
                if(data?.Region__c && this.salesOrgCode!='1410'){
                ...
                }else{ // SKI : Edited by Sandeep Vishwakarma AF - Material
                    
                    if(data?.Zone__r?.ZMMUser__c && data?.Zone__r?.ZMMUser__r?.IsActive){
                        this.freesampleObj.Office_Manager__c = data.Zone__r.ZMMUser__c;
                    }else{
                        if(data?.Zone__r?.ZonalHead__c && data?.Zone__r?.ZonalHead__r?.IsActive)
                            this.freesampleObj.Office_Manager__c = data.Zone__c!=undefined?data.Zone__r.ZonalHead__c:'';
                    } 
                }
                if(data?.Zone__c){
                ...
                // SKI : Edited by Sandeep Vishwakarma AF - Material
                if(this.salesOrgCode!='1410' && data?.Zone__c && data?.Zone__c!=undefined){
                    //INC0475519 - SFDC - No Approval history for sample request- could you please check
                    //this.freesampleObj.Sales_Assistant__c  = data?.Zone__c!=undefined?data.Zone__r.ZMMUser__c:data.Zone__r.ZonalHead__c;
                    if(data?.Zone__r?.ZMMUser__c && data?.Zone__r?.ZMMUser__r?.IsActive){
                        console.log('data.Zone__r.ZMMUser__c >>>>>>>>>>>.'+data.Zone__r.ZMMUser__c);
                        this.freesampleObj.Sales_Assistant__c = data.Zone__r.ZMMUser__c;
                    } else if(data?.Zone__r?.ZonalHead__c && data?.Zone__r?.ZonalHead__r?.IsActive){
                        console.log('data.Zone__r.ZonalHead__c >>>>>>>>>>>.'+data.Zone__c!=undefined?data.Zone__r.ZonalHead__c:'');
                        this.freesampleObj.Sales_Assistant__c = data.Zone__c!=undefined?data.Zone__r.ZonalHead__c:'';
                    }
                }else{
                    this.freesampleObj.Sales_Assistant__c  = data?.SBU__c!=undefined?data.SBU__r?.SBUMarketingheads__c:'';
                    if(!data?.SBU__r?.SBUMarketingheads__c){
                        this.freesampleObj.Sales_Assistant__c  = data.SBU__c!=undefined?data.SBU__r?.SBUUser__c:'';
                    }
                    console.log('data?.SBU__r?.SBUMarketingheads__c ',data?.SBU__r?.SBUMarketingheads__c, 'data.SBU__r?.SBUUser__c ',data.SBU__r?.SBUUser__c);
                }
            }).catch(err=>{
    ...
    if(this.lstfreeSampleProduct.length>0){
        ...
        if(this.freesampleObj.Status__c==this.status.Pending_from_Depot && this.freesampleObj.Sub_Status__c==this.sub_status.for_PO_Number){
            ...
            if(this.freesampleObj.Material_Dispatched_Date__c!=undefined){
                let dispatchedDate = new Date(this.freesampleObj.Material_Dispatched_Date__c);
                let d = new Date();
                console.log('Dispatched Date  ',new Date(dispatchedDate.getFullYear(),dispatchedDate.getMonth(),dispatchedDate.getDate()),' Today ',new Date(d.getFullYear(),d.getMonth(),d.getDate()));
                if(this.freesampleObj.Material_Dispatched_Date__c!='' && new Date(dispatchedDate.getFullYear(),dispatchedDate.getMonth(),dispatchedDate.getDate()) >= new Date(d.getFullYear(),d.getMonth(),d.getDate())){
                    this.showCustomToast('Success','Success','success');
                    this.saveMaterialReq();
                }else{
                    this.showCustomToast('Error','Error','error');
    ...
    saveMaterialReq(){
        ...
        if(this.freesamplemanagementid==''){ // Create mode
            ...
            if(this.profiles.regional_and_zonal_manager?.includes(this.profile_name)){
        ...
        if(this.profile_name==this.profiles.territory_manager || this.profiles.regional_and_zonal_manager?.includes(this.profile_name) || this.profile_name==this.profiles.FMM_Users){    
