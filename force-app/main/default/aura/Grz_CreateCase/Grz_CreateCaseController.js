({
    doInit : function(component, event, helper) {
         var labelAccepted =[];
        labelAccepted = $A.get("$Label.c.Grz_CaseFileTypeAccepted").split(',');
        component.set("v.fieldTypeShow",$A.get("$Label.c.Grz_CaseFileTypeAccepted"))
         component.set("v.filetype",labelAccepted);
    ...
    createCaseData : function(component, event, helper){
        ...
        if (caseCreate.Subject == '') {
            errormsg = ' Subject';
            component.set("v.errorsubject",'*Please enter the Subject');
            document.getElementById("casebtn").disabled = false;
            // return true;
        }
        else{
            if(caseCreate.Subject.length > 255){
                //component.set("v.errorsubject",'* Subject length should be less than 255 ');
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    "title": "Error!",
                    "message": "Subject length should be less than 255",
                    "type" : "error"
                });
                toastEvent.fire();
                document.getElementById("casebtn").disabled = false;
        ...
        if(caseCreate.Description == '') {
            ...
            component.set("v.errorDescription",'*Please provide the Description');
            ...
            if(caseCreate.Description.length > 32000){
                //component.set("v.errorDescription",'* Description length should be less than 32000 ');
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    "title": "Error!",
                    "message": "Description length should be less than 32000",
                    "type" : "error"
                });
                toastEvent.fire();
                document.getElementById("casebtn").disabled = false;
        ...
        if(internaluser){
            if (accountid == null || accountid == '') {
                ...
                component.set("v.erroraccount",'*Please enter the Account');
                document.getElementById("casebtn").disabled = false;
                // return true;
            }
            else{
        ...
        /*if(caseCreate.ContactPhone == ''){
            console.log('test');
            alert('Please provide the valid Phone Number'); 
            document.getElementById("casebtn").disabled = false;
            return true;
        }
        else if(caseCreate.ContactEmail == ''){
            console.log('test');
        ...
        if(errormsg != '')
        {
            ...
            errormsg = 'Required field(s) missing:' + errormsg;
        ...
        action.setCallback(this, function(response) {
            ...
            if (state == "SUCCESS") {
                ...
                toastEvent.fire();
                //window.location.href = "/uplpartnerportal/s/casedetailpage?id=" + response.getReturnValue().Id;
                ...
                toastEvent.setParams({
                    "title": "ERROR!",
                    "message": "The Case cannot be created.",
                    "type" : "error"
                });
                toastEvent.fire();
            }
    ...
    Cancel:function(component, event, helper) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state == "SUCCESS") {
                location.href = "/uplpartnerportal/s/";
