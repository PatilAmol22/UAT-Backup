({
    ...
    doInit: function (component, event, helper) {
        ...
        helper.fetchSpainOrgs(component,helper);
        var orgValue = component.get("v.Selected_Org");
        console.log("orgValue ->", orgValue);
        orgValue = "5310;5320;";
        if ($A.util.isUndefined(searchKey) || searchKey == '') {
            console.log("TOday- IF ->");
            helper.fetchRequirements(component, page, recordToDisply, searchDepot,orgValue);//   Called this inside fetch depot
        } else {
            console.log("TOday- ELSE ->");
            helper.searchRequirements(component, searchField, searchKey, page, recordToDisply, searchDepot,orgValue);
        }
    },
    ...
    /*onDepotChange: function (component, event, helper) {
        ...
        if (searchDepot != null || searchDepot != '') {
            if (!$A.util.isUndefined(searchKey)) {
                helper.fetchRequirements(component, page, recordToDisply, searchDepot,orgValue);
            } else {
                searchKey = '';
        ...
        //This function is called when the Org selection is changed
    onOrgChange : function(component, event, helper){
        var orgValue = component.get("v.Selected_Org");
        console.log('Sayan100 onOrgChange Org is--->'+orgValue);
        //var searchDepot = component.find("depot_items").get("v.value");
        var searchDepot = "CR00";
        console.log('Sayan100 onOrgChange Depot--->'+searchDepot);
        if(orgValue == '5310' || orgValue == '5320'){
            component.set("v.Selected_Depots","CR00");
        }
        //Copy
        var searchKey = component.get("v.selItem"); //event.getParam("term");
        console.log('->On Depot Chnage');
        //var searchDepot = component.find("depot_items").get("v.value");
        var searchDepot = "CR00";
        console.log('Sayan100 onDepotChange depot--->'+searchDepot);
        // var opts = component.find("select_id").get("v.value");
        var opts = document.querySelector('input[name="options"]:checked').value;
        console.log('->On Depot Chnage');
        var recordToDisply = component.find("recordSize").get("v.value");
    ...
    searchKeyChange: function (component, event, helper) {
        ...
        if ($A.util.isUndefined(searchKey) || searchKey == '') {
            ...
            helper.searchRequirements(component, helper, searchField, searchKey, page, recordToDisply,orgValue);
        }
