({
    ...
    getOrderType : function(component,event){
        var opts=[{"class": "optionClass", label: $A.get("$Label.c.Industrial_Order"), value: "Industrial Order", selected: "true"}];
        component.find("orderTypeOptions").set("v.options", opts); 
        
    },
    
    showCmpsOnNormalOrder : function(component,event){
        component.set('v.newSalesOrder.Order_Type_Italy__c','Industrial Order');
    ...
    editForm:function(component,event){
        ...
        if(orderTypeItaly!=undefined){
            if(orderTypeItaly!="Child Order" && orderTypeItaly!="Industrial Order"){
        ...
        if(soId!=null && soId!=undefined){
            ...
            action.setCallback(this, function(a) {
                ...
                if(state == "SUCCESS") {
                    ...
                    var getOrderType = 'Industrial Order';
                    ...
                    if(orderItemList[idx].typeOfProduct=="Vendita"){
                        if(IscustTCAvailable){
                            ...
                            if(expressOrder && fRAvailable){
                                ...
                                if(orderItemList[idx].palletSize <= orderItemList[idx].qty){//
                                    console.log('More than pallet 1');
        ...
        if(productType=='Omaggio'){
            ...
            if(IscustTCAvailable){
                ...
                if(expressOrder && fRAvailable){
                    ...
                    if(palletSize<=qty){
                        console.log('palletSize less than qty');
                        transContri =TCParam.MoreThanPallet__c*qty;
                        component.set("v.SingleOrderItem.TransContribution",transContri);
                        component.set("v.SingleOrderItem.TransContribution2",transContri);
                        
                    }else if(palletSize>qty){
                        console.log('palletSize greater than qty');
    ...
    validateSingleOrderItem :function(component){
        try{
            ...
            var getBasePrice = component.find("matPrice");
            console.log('Amit->'+getBasePrice.get("v.value"));
            var toastMsg = '';
            var flag = true;
            var oLIPIMap={};
            ...
            if(oLIPIMap.hasOwnProperty(orderItem.productId +'-'+orderItem.typeOfProduct)){
                ...
                if(!getBasePrice.get("v.value") || getBasePrice.get("v.value") == '0'){
                    flag = false;
                    component.find("matPrice").set("v.errors",[{message: $A.get("$Label.c.Please_enter_Base_Price")}]);                
                }else{
                    component.find("matPrice").set("v.errors",null);
                }
    ...
    createSalesOrder : function(component, event, status) {
        ...
        if(isValid && isValidItems){
            ...
            if(status=="Submitted") {
                ...
                component.set("v.disableThis",true);
                console.log('statusVItaly before : '+status);
            }else if(status== "Draft") {
                //Execute to save as draft without flag check/rollback
                console.log('statusVItaly : '+status);
