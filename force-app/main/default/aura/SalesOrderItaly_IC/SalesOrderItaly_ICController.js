({
    ...
    handleBasePrice: function(component,event,helper){
        var target2 = event.getSource();
        var target2val = target2.get("v.value");
        var target = component.find("rebId1");
        var pgCode = component.get("v.pgCodeId");
        var customerCode = component.get("v.customerCodeId");
        console.log(pgCode+'pgCode');
        ...
        var netInvPrice1 = 0;
        var netPrice = 0;
        if(target2val!=0 && target2val!=null){
            var inputCmpNew = event.getSource();
            inputCmpNew.set("v.errors", null);
            $A.util.removeClass(inputCmpNew, "error");
            ...
            var errorMessage= $A.get("$Label.c.Please_enter_Base_Price");
            target2.set("v.errors", [{message:errorMessage}]);
            $A.util.addClass(target2, "error");
        }
   ...
   /* validateQtylength  : function(component, event, helper) {
    ...
    updateTableRow1 : function(component, event, helper) {
        ...
        var netInvPrice = target.get("v.requiredIndicatorClass");
        var errorMessage = '';
        console.log('Sayan unitValue->'+unitValue);
        console.log('Sayan netInvPrice->'+netInvPrice);
        if(unitValue){
            /*if(netInvPrice < unitValue){
                target.set("v.errors", [{message:$A.get("$Label.c.Please_enter_valid_Estimated_Final_Price")}]);
                target.set("v.value",'');
    ...
    handlePaymentChange : function(component, event){
        ...
        console.log('Sayan PaymentTermMap->'+JSON.stringify(component.get("v.PaymentTermMap")));
        console.log('Sayan paymentTerm->'+component.get("v.paymentTerm"));
        ...
        //if(IsPaymentTermAvailable){
            if(paymentTerm!='None'){
                ...
                console.log('Sayan selected PT id->'+ptObjId);
                console.log('Sayan selected PT disc->'+ptObjDic);
