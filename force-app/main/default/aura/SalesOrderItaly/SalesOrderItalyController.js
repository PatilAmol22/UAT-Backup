({
    ...
    doInit: function(component, event, helper) {
        ...
        window.setTimeout($A.getCallback(function() {helper.getOrderFields(component);}),2000 );
         component.set("v.orderTypeCode","Normal Order");//Added by GRZ(Pragati Sharma) for RITM0419632 on 20-10-2022
    ...
    closeMinimumQtyDialog : function(component, event, helper) {
        helper.toggleLessQuantityDialog(component);
    ...
    handleOrderTypeChange : function(component, event, helper) {
        ...
        //Added by GRZ(Pragati Sharma) for RITM0419632 on 20-10-2022
        component.set("v.orderTypeCode", selectOrderType);
        //console.log("v.orderTypeCode"+orderTypeCode);
    ...
    placeChildOrder: function(component, event, helper) {
        //var selectOrderType = component.find('orderTypeOptions').get("v.value"); 
        var selectOrderType = component.get("v.orderTypeNames"); //Divya: 11-01-2020: Added for SCTASK0351764
        console.log('moumita selectOrderType :'+selectOrderType);
        
        if(selectOrderType=='ZZCQ'){
            component.set("v.isChildOrderType",true);
            helper.reloadSOFromParent(component);
            //Added by Varun Shrivastava Start : SCTASK0360650
            helper.resetOrderItemList(component);
            ////Added by Varun Shrivastava End : SCTASK0360650
            window.setTimeout($A.getCallback(function() {helper.reloadSOParentItems(component);}),500 );
        }
    },
    //Logic to show confirmation dialog with Simulation Flag & Message on Save and Submit.
    ...
    editForm : function(component, event, helper) {
        ...
        if(component.get("v.orderType2")=='Child Order'){
            //helper.reloadSOChildOrderOnEdit(component);
            
            window.setTimeout($A.getCallback(function() {helper.reloadSOChildItemsOnEdit(component);}),500 );
        }else{
    ...
    tabActionClicked: function(component, event, helper){
        ...
        var showDel = component.get("v.showDeliveryDate"); // SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022...
        ...
        if(actionId == 'selectproduct'){
            ...
            //Added by GRZ(Shubham Bansal)for RITM0419628 on 09-09-2022       
            if(component.get("v.minimumQty")!=itemRow.minQty)   {
                  component.set("v.minimumQty",0);
            }  
            if(itemRow.minQty!=null)
            {
               component.set("v.minimumQty",itemRow.minQty);
               component.set("v.skuDesc",itemRow.skuDescription);
               
            }
          //End RITM0419628 on 09-09-2022
            ...
            component.set("v.SingleOrderItem.shipDate",currentDt);
            /* -------------- Start SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022 ----------- */
            if(showDel == true){
                component.set("v.SingleOrderItem.deliveryDate",currentDate);
            }
            else{
                component.set("v.SingleOrderItem.deliveryDate",'');
            }
            /* ------------- End SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022 ---------------- */
            ...
            if(qtyString!=''){
                ...
                if(component.get("v.orderType2")!=undefined){
                    ...
                    if(component.get("v.orderType2")=='Parent Order'){
                        // Added If Condition vijay laxmi -ve
                        if(component.get("v.isChildOrderType")==true && ((parseFloat(orderItemList[rowIndex].balanceQtyStr)<0 || orderItemList[rowIndex].balanceQtyStr=='0.00' )|| parseFloat(orderItemList[rowIndex].balanceQtyStr)>0 )){
                            console.log(' Balance is 0 or -ve');
                            balancQTY=0;
                        ...
                        balancQTY=0;
                        balancQTY=orderItemList[rowIndex].balanceQty;
                         console.log('Vijaylaxmi Balance is blank'+balancQTY);
                    }
                }
    ...
    poDateChange:function(component,event,helper){
        var target = event.getSource();
        console.log('target ' + target);
        console.log('target.get("v.value") ' + target.get("v.value"));
        if(target.get("v.value") == null){
            component.set("v.newSalesOrder.Purchase_Order_Date__c",null);
        }
        else{
            component.set("v.newSalesOrder.Purchase_Order_Date__c",target.get("v.value"));
        }
    }
    /* -------------------- End SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022 ---------------------------------- */
