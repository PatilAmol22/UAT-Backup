({
    ...
    getOrderFields : function(component, event) {
        ...
        action.setCallback(this, function(a) {
            ...
            if (state == "SUCCESS") {
                ...
                console.log(returnValue.TransContributionParam);
   ...
   getOrderType : function(component,event){
        //{"class": "optionClass", label: $A.get("$Label.c.None"), value: 'None' },
        /*{"class": "optionClass", label: "Normal Order", value: "Normal Order", selected: "true"},
                  {"class": "optionClass", label: "Parent Order", value: "Parent Order"}*/
        var profileName = component.get("v.user.Profile.Name");2
        // var opts=[      //Start GRZ(Dheeraj Sharma) : RITM0427350  Date :20-09-202
            //Sayan 17th Sept           //Start GRZ(Dheeraj Sharma) : RITM0427350  Date :20-09-2022
       //     {"class": "optionClass", label: $A.get("$Label.c.Normal_Order"), value: "Normal Order", selected: "true"}];      //Start GRZ(Dheeraj Sharma) : RITM0427350  Date :20-09-2022
      //  if(profileName == 'Customer Service & Finance Manager Italy'){       //Start GRZ(Dheeraj Sharma) : RITM0427350  Date :20-09-2022
        var opts=[
            //Sayan 17th Sept
            {"class": "optionClass", label: $A.get("$Label.c.Normal_Order"), value: "Normal Order", selected: "true"} ,
            {"class": "optionClass", label: $A.get("$Label.c.Parent_Order"), value: "Parent Order"}];//RITM0259168: Enabled Parent Order as Asked by Ashutosh, 20th Sept, 2021
    ...
    showCmpsOnNormalOrder : function(component,event){
        component.set('v.newSalesOrder.Order_Type_Italy__c','Normal Order');
    ...
    showCmpsOnParentOrder : function(component,event){
        ...
        component.set('v.showSaveDraft',true); //Moumita made Draft button visible SCTASK0308602(RITM0141480) for Campaign orders
        ...
        //component.find("endDate").set('v.value', currentDate);
    },
    ...
    editForm:function(component,event){
        ...
        if(orderTypeItaly!=undefined){
            if(orderTypeItaly!="Child Order" && orderTypeItaly!="Normal Order"){
                console.log('print orderTypeItaly')
    ...
    childOrderEditForm:function(component,event){ //Added by Azhar for campaign module
        ...
        if(orderItemList.length >=1){
            var orderTypeItaly=component.get("v.orderType2");
            //console.log('orderTypeItaly childOrderEditForm :'+orderTypeItaly)
            if(orderTypeItaly!="Child Order" && orderTypeItaly!="Normal Order"){
                component.set("v.disableExpress",false); 
            }else{
            ...
            //console.log('disableExpress :'+component.get("v.disableExpress"));
        }
        ...
        if(accId!=null){
            ...
            action.setCallback(this, function(a) {
                ...
                if(state == "SUCCESS") {
                    ...
                    component.set("v.newSalesOrder.Purchase_Order_Date__c",currentDt); 
                    /* ------------ End SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022 --------------- */
                    ...
                    component.set("v.orderType","Normal Order"); ////Added by GRZ(Pragati Sharma) for RITM0419632 on 20-10-2022
                    
                    //console.log('orderTypeItaly '+cartData.orderTypeItaly);
                    component.set("v.newSalesOrder.Order_Type_Italy__c",cartData.orderTypeItaly);
                    if(cartData.orderTypeItaly=="Parent Order"){
    ...
    reloadSO: function(component){
        ...
        var OrderType=component.get("v.orderTypeCode");
        console.log('ordetype'+OrderType);
        
        console.log('soId is '+soId);
        ...
        if(soId!=null && soId!=undefined){
            ...
            action.setCallback(this, function(a) {
                ...
                if(state == "SUCCESS") {
                    ...
                    /* --------------- Start SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022 ------- */
                    if(salesOrder.Purchase_Order_Date__c == null){
                        var crDt = new Date();
                        var d = (crDt.getDate() < 10 ? '0' : '') + crDt.getDate();
                        var M = ((crDt.getMonth() + 1) < 10 ? '0' : '') + (crDt.getMonth() + 1);
                        var y = crDt.getFullYear();
                        var currentDt = (y + "-" + M + "-" + d);
                        salesOrder.Purchase_Order_Date__c = new Date(currentDt);
                    }   
                    /* ------------ End SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022 --------------- */
                    component.set("v.newSalesOrder.Purchase_Order_Date__c",salesOrder.Purchase_Order_Date__c);
                    ...
                    var getOrderType = salesOrder.Order_Type_Italy__c;
                    ...
                    if(getOrderType=='Parent Order'){
                        ...
                        //this.showCmpsOnParentOrder(component,event);
                        this.reloadSOParentZeroBalanceQtyItems(component);
                        component.set('v.newSalesOrder.Order_Type_Italy__c','Parent Order');
                        component.set('v.showStartEndDate',true);
                        component.set('v.showSaveDraft',false);
                        var today = new Date();
                    ...
                    }else if(getOrderType=='Return Order'){
                   component.set("v.ReturnorderType", getOrderType);
                        component.set("v.ReturnorderTypeCheck", true);
                  // opts.push({"class": "optionClass", label: $A.get("$Label.c.None"), value: 'None'});  

                 component.find("returnorderTypeOptions").set("v.options", {"class": "optionClass", label: 'Richiesta di reso', value: 'Return Order'}); 
                      //  alert('helper'+component.get("v.ReturnorderType")+''+getOrderType);
                    }else{
    ...
    // 2. Called in getOrderFields() after all Fields are loaded.
    // 3. Uses recordId to fetch related data of a SO record.
    reloadSOFromParent: function(component){
        component.set("v.headerMsg", $A.get("$Label.c.Create_Order"));
        var action = component.get("c.getSalesOrderFromParent");
        var soId = component.get("v.sOId");
        var emptyPT = component.get("v.emptyPaymetTerm"); 
        var isPDdisabled = component.get("v.isPaymentDiscDisable");
        ...
        if(soId!=null && soId!=undefined){
            ...
            action.setCallback(this, function(a) {
                ...
                if(state == "SUCCESS") {
                    ...
                    var getOrderType = salesOrder.Order_Type_Italy__c;
                    component.set('v.orderType',getOrderType);
                    console.log('orderr'+getOrderType);
                    var getStartDate = salesOrder.Valid_From__c;
                    var getEndDate = salesOrder.Valid_To__c;
                    component.set("v.disableOnExpress",true);
                    
                    if(getOrderType=='Child Order'){
                        console.log('salesOrder.Sales_Order__r.Name : '+salesOrder.Sales_Order__r.Name);
                        component.set('v.parentOrderNo',salesOrder.Sales_Order__r.Name);
                        component.set("v.grossNetPrice", salesOrder.Total_Amount__c);
                        this.childOrderForm(component);
                    }else{
            ...
            $A.enqueueAction(action);
            this.childOrderEditForm(component);
        }else{
            console.log('so id not found');
        }
    },
    ...
    // 2. Called in getOrderFields() after all Fields are loaded.
    // 3. Uses recordId to fetch related data of a SO record.
    reloadSOChildOrderOnEdit: function(component){
        component.set("v.headerMsg", $A.get("$Label.c.Create_Order"));
        var action = component.get("c.getSalesOrderFromParent");
        var soId = component.get("v.sOId");
        var emptyPT = component.get("v.emptyPaymetTerm"); 
    ...
    reloadSOParentZeroBalanceQtyItems: function(component){
        console.log('reloadSOParentZeroBalanceQtyItems');
        ...
        if(component.get("v.sOId")!=null){
            ...
            action.setCallback(this, function(a) {
                var state = a.getState();
                if(state == "SUCCESS") {
                    console.log('state reloadSOParentZeroBalanceQtyItems  : '+state)
                    component.set("v.orderItemList2", a.getReturnValue());
                    var orderItemList = component.get("v.orderItemList2");
                    var sumQty=0;
                    for (var idx = 0; idx < orderItemList.length; idx++) {
                        if(orderItemList[idx].typeOfProduct=='Vendita'){
                            sumQty = sumQty + orderItemList[idx].balanceQty;
                        }
                    }
                    console.log('sumQty of vendita : '+sumQty)
                    if(sumQty==0){
                        component.set("v.placechildOrderDisabled",true); 
                    }
                }
        ...
        if(component.get("v.sOId")!=null){
            ...
            action.setCallback(this, function(a) {
                ...
                if(state == "SUCCESS") {
                    ...
                    //Added by Varun Shrivastava End : SCTASK0360650
                    console.log('After Child Order JSON : '+JSON.stringify(orderItemResult));
                    console.log('After Child Order JSON1 : '+JSON.stringify(orderItemResult1));
                }
                else{
                    var toastMsg = $A.get("$Label.c.Error_while_reloading_Order_Items");
                    this.showErrorToast(component, event, toastMsg);                      
    ...
    reloadSOChildItemsOnEdit: function(component){
        console.log('getSalesOrderItemsOnEditChildOrder')
        
        var action = component.get("c.getSalesOrderItemsOnEditChildOrder");
        if(component.get("v.sOId")!=null){
            console.log('sOId  :'+component.get("v.sOId"));
            console.log('parentOrderNo  :'+component.get("v.parentOrderNo"));
            action.setParams({
                soId: component.get("v.sOId"),
                "parentOrderNo":component.get("v.parentOrderNo")
            });
    ...
    fetchSKUData : function(component, event) {
        ...
        var accId =  component.get("v.recordId");
        var orderType=component.get("v.orderTypeCode");//Added  by GRZ(Pragati Sharma) for RITM0419632 on 20-10-2022
        ...
        action.setParams({ 
            "accId":accId,
            "pgCode": pgCode,
            "orderType":orderType //Added by GRZ(Pragati Sharma) for RITM0419632 on 20-10-2022
        });  
    ...
    updateRowCalculations1 : function(component, event, helper){
        ...
        console.log('TCParam1'+ JSON.stringify(TCParam.LessThanPallet__c));
        ...
        if(productType=='Omaggio'){
                ...
                if(expressOrder && fRAvailable){
                    ...
                    if(palletSize<=qtysum){
                        ...
                        if(sRAvailable){
                            ...
                            //component.set("v.SingleOrderItem.TransContribution",transContri);
            ...
            if(IscustTCAvailable){
                if(expressOrder){
                    ...
                    component.set("v.SingleOrderItem.TransContribution",expressDCostTC*qty);
                    component.set("v.SingleOrderItem.TransContribution2",expressDCostTC*qty);
                }else{
                    console.log('normalDCostTC'+normalDCostTC);
                    component.set("v.SingleOrderItem.TransContribution",normalDCostTC*qty);
        ...
        try{
            ...
            if(orderItemList.length > 0){
                ...
                for (i = 0; i < orderItemList.length; i++) {
                    ...
                    if(showDel == true){
                        if(reqDel == true && (obj.deliveryDate == null || obj.deliveryDate == '')){
                            error = true;
                            errMsg = $A.get("{!$Label.c.Delivery_Date_is_Required}");
                            break;
                        }
                        else if(obj.deliveryDate != null || obj.deliveryDate != ''){
    ...
    validateOrder: function(component){
        ...
        var showPo = component.get("v.showPODate");  // SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022..
        var poReq = component.get("v.isPODateReq");  // SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022..
        ...
        component.set("v.newSalesOrder.Order_Type_Italy__c",selectOrderType);
        ...
        if(paymentTermId){
            ...
            if(!paymentTermId.get("v.value") || paymentTermId.get("v.value") == 'None' || paymentTermId.get("v.value") == 'Nessuno' || paymentTermId.get("v.value") == undefined) {
                flag = false;
        ...
        /* ------------------ Start SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022 ----------------- */
        if(showPo == true){
            var dt = component.get("v.newSalesOrder.Purchase_Order_Date__c");
            if(poReq == true && (dt == null || dt == '')){
                flag = false;
                component.find("po_date").focus();
                var toastEvent1 = $A.get("e.force:showToast");
                  var titl  = $A.get("{!$Label.c.Error}");
                  var msg = $A.get("{!$Label.c.Purchase_Order_date_is_required}");
                  toastEvent1.setParams({
                      "title": titl,
                      "type": "Error",
                      "message": msg
                ...
                component.set("v.newSalesOrder.Purchase_Order_Date__c",new Date(dt));
            }
        }
        else{
            component.set("v.newSalesOrder.Purchase_Order_Date__c",null);
        ...
        try{
           ...
           /* -------------- End SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022 ---------------- */            
            ...
            var getUnitValueId = component.find("estimatedFinal1");
             var minQty = component.get("v.minimumQty");  
            var skuDescription=component.get("v.skuDesc");
            var warningMessage='';
            ...
            if(oLIPIMap.hasOwnProperty(orderItem.productId +'-'+orderItem.typeOfProduct)){
                ...
                if(orderItem.typeOfProduct=='Omaggio'){//Changed by Azhar on 31 Oct 2020 CR#123
                  ...
                  if(getQtyId.get("v.value")<minQty) //Added by Shubham Bansal for RITM0419628 on 09-09-2022
                { 
                     flag = false;
                   
                      warningMessage= skuDescription+" "+ $A.get("$Label.c.SKU_should_have_minimum_quantity")+" "+minQty+". "+$A.get("$Label.c.Please_Enter_value_greater_than_this");
                    component.set('v.LessQuantityfDialogMessage',warningMessage);
                    this.toggleLessQuantityDialog(component);
                ...
                //component.set('v.LessQuantityfDialogMessage',null);
                 if(getQtyId.get("v.value")<minQty) //Added by Shubham Bansal for RITM0419628 on 09-09-2022
                {
                     flag = false;
                ...
                if(!getShipDtId.get("v.value")) {
                    ...
                    var x = new Date(selectedDate);
                    var yy = new Date(currentDt);   // SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022...
                    ...
                    if (+x < +yy) { // SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022...
                ...
                if(showDel == true){
                    ...
                    if(reqDel == true && dt == ""){
                        ...
                        orderItem.deliveryDate = z;
                    }
                    component.set("v.SingleOrderItem",orderItem);
                }
            }
            if(error == true){
                var toastEvent1 = $A.get("e.force:showToast");
    ...
    validateOrderItems: function(component){
        try{
           //debugger; 
            ...
            var error = false;
            var errMsg = ''; 
            var today2 = new Date(today.getTime() + 86400000); // for current date + 1 ...
            var dd = (today2.getDate() < 10 ? '0' : '') + today2.getDate();
            var MM = ((today2.getMonth() + 1) < 10 ? '0' : '') + (today2.getMonth() + 1);
            var yyyy = today2.getFullYear();
            var currentDt = (yyyy + "-" + MM + "-" + dd); 
            var z = '';
            var y2 = new Date(currentDt);
            /* ------------------ End SKI(Nik) : #CR155 : Brazil Margin Block : 12-09-2022 ----------------- */
            ...
            //Updated by Varun Shrivastava start : SCTASK0360650
            if(component.get("v.isChildOrderType")){  //  || component.get("v.isChildOrderType"
                orderItemList = component.get("v.orderItemListTest");
            }
            //Updated by Varun Shrivastava start : End
            var getProductId = component.find("itemproduct1");
            ...
            if(orderItemList.length <= 0){
                ...
                for(var j = 0; j <orderItemList.length; j++){
                    if(showDel == true){
                        var obj = new Object(orderItemList[j]); 
                        var dt = obj.deliveryDate;
                        if(dt == null || dt == ''){
                            dt = '';
                            z = '';
                        }
                ...
                if(isProductArray){
                    ...
                    if(getProductType.get('v.value')=='Vendita'){
                        if(!getProductId.get("v.value")) {
                            flag = false;
                            console.log('Vivek 2370');
                            component.find("itemproduct").set("v.errors",[{message: $A.get("$Label.c.Product_is_required")}]);
                            console.log('Vivek 2372');
                        }
                        else{
                            console.log('Vivek 2375');
                            console.log('Vivek->'+component.find("itemproduct"));
                            //component.find("itemproduct").set("v.errors",null);
                            console.log('Vivek 2377');
                        }
                        
                        if(!getShipDtId.get("v.value")) {
                            if(isChildOrder==true){
                                if(!getQtyId.get("v.value") || getQtyId.get("v.value") == '0') {
                                    console.log('Vivek 2383');
                                    component.find("shipDt").set("v.errors",null);
                                    console.log('Vivek 2385');
                                }else{
                                    var selectedDate = getShipDtId.get("v.value");
                                    ...
                                    console.log('currentDate: '+y);
                                    if (+x < +y) {
                                        console.log('Vivek 2401');
                                        component.find("shipDt").set("v.errors", [{message:$A.get("$Label.c.Date_of_Shipment_cannot_be_less_than_today")}]);
                                        console.log('Vivek 2403');
                                        flag = false;
                                    } 
                                }
                            }else{
                                flag = false;
                                console.log('Vivek 2409');
                                component.find("shipDt").set("v.errors",[{message:$A.get("$Label.c.Please_Enter_Shipping_Date")}]);
                                console.log('Vivek 2411');
                            }
                        }
                        else{
                            var selectedDate = getShipDtId.get("v.value");
                            ...
                            console.log('currentDate: '+y);
                            if (+x < +y) {
                                console.log('Vivek 2429');
                                component.find("shipDt").set("v.errors", [{message:$A.get("$Label.c.Date_of_Shipment_cannot_be_less_than_today")}]);
                                console.log('Vivek 2431');
                                flag = false;
                            }
                            else{
                                console.log('Vivek 2435');
                                component.find("shipDt").set("v.errors",null);  
                                console.log('Vivek 2437');
                            }
                        }
                        
                        if(!getQtyId.get("v.value") || getQtyId.get("v.value") == '0') {
                            flag = false;
                            console.log('Vivek 2443');
                            component.find("itemqty").set("v.errors",[{message: $A.get("$Label.c.Please_enter_Quantity")}]);
                            console.log('Vivek 2445');
                        }
                        else{
                            console.log('Vivek 2448');
                            component.find("itemqty").set("v.errors",null);
                            console.log('Vivek 2450');
                        }
                        ...
                        if(getUnitValueId){
                            ...
                            if(isModDraft==false){
                                ...
                                if((!getUnitValueId.get("v.value") || getUnitValueId.get("v.value") == '0')&&(orderItemList[0].typeOfProduct=="Vendita")){
                                    flag = false;
                                    console.log('Vivek 2464');
                                    component.find("estimatedFinal").set("v.errors",[{message:$A.get("$Label.c.Please_Enter_Estimated_Final_Price")}]);
                                    console.log('Vivek 2466');
                                }
                                else{
                                    console.log('Vivek 2468');
                                    component.find("estimatedFinal").set("v.errors",null);
                                    console.log('Vivek 2471');
                                }
                            }
                            else{
                                console.log('*isModifyDraft else - > '+isModDraft);
                                console.log('*New modify Loop');
                                if(getUnitValueId[0] && (!getUnitValueId[0].get("v.value") || getUnitValueId[0].get("v.value") == '0')&&(orderItemList[0].typeOfProduct=="Vendita")){
                                    flag = false;
                                    console.log('Vivek 2479');
                                    component.find("estimatedFinal")[0].set("v.errors",[{message:$A.get("$Label.c.Please_Enter_Estimated_Final_Price")}]);
                                    console.log('Vivek 2481');
                                }
                                else{
                                    console.log('Vivek 2484');
                                    if(component.find("estimatedFinal")[0]){
                                        component.find("estimatedFinal")[0].set("v.errors",null);
                                    }
                                    console.log('Vivek 2486');
                                }  
                            }
                        }
                    }
    ...
    addTableRow : function(component,helper,isValidItems){
        var orderItemList = component.get("v.orderItemList");
        var orderItem = JSON.parse(JSON.stringify(component.get("v.SingleOrderItem")));
    ...
    createSalesOrder : function(component, event, status) {
        ...
        console.log('isValidItems ' + isValidItems);
        if(isValid && isValidItems){ // isValid || isValidItems
            ...
            action.setParams({ 
                ...
                "recordId":component.get("v.recordId")  //Start GRZ(Buesh Singla) : INC0384571  Date :03-10-2022
            });   
    ...
    toggleDialog: function(component){
     ...
     //Added by GRZ(Shubham Bansal)for RITM0419628 on 09-09-2022 
     toggleLessQuantityDialog: function(component){
        
        var dialog = component.find("modalforQuantity");
        $A.util.toggleClass(dialog, "slds-hide");
          //$A.util.toggleClass(modal, 'slds-fade-in-open');
        
        var backdrop4 = component.find("backdropQty");
        $A.util.toggleClass(backdrop4, "slds-hide"); 
