  ...
  textKeyChange: function(component, termtext) {
    ...
    if ($A.util.isUndefined(searchKey) || searchKey == "") {
      ...
      if (opts == "Product/Sku" || opts == "Product/SKU") {
        ...
        if (searchKey != null) {
          console.log(
            "Helper Product SKU Inside ELSE Storage Location searchKey-> " +
              searchKey
          );
          this.searchRequirements(
            component,
    ...
    action.setCallback(this, function(response) {
      ...
      if (state == "SUCCESS") {
        ...
        for (var i = 0; i < returnValue.length; i++) {
          opts.push({
            class: "optionClass",
            label: returnValue[i].Depot_Code__c,
        ...
        console.log(
          "component.get(v.Selected_Depots)--->" +
            component.get("v.Selected_Depots")
        );
  ...
  searchRequirements: function(
    ...
    action.setCallback(this, function(response) {
      ...
      if (state === "SUCCESS") {
        ...
        component.set(
          "v.pages",
          Math.ceil(returnValue.total / recordToDisply1)
        );
