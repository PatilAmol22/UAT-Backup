...
//        console.log('tselectedRIDIddoInt:-'+component.get("v.selectRecordId"));
        if(component.get("v.sORecordId") && component.get("v.recordPageFlag")){
            window.setTimeout($A.getCallback(function() {helper.getOrderFields(component);}),2000 );
    ...
    changeCurrency : function(component, event, helper) {
        var curr = component.get("v.solItemCurrency");//CR#174 -Chile Margin Block-SKI- kalpesh chande -  23/01/2023 
                                ...
                                openDialog : function(component, event, helper) {
                                    ...
                                    addTableRow : function(component, event, helper){
                                       ...
                                       // console.log('isValid>>--->'+isValid);
                                        ...
                                        var LastValue =0;
                                        var orderItemList = component.get("v.orderItemList");
                                       // console.log('Test AddTable Row:-'+JSON.stringify(orderItemList));
                                       ...
                                       // console.log('orderItemList.length'+orderItemList.length);
                                        ...
                                        normalremoveTableRow : function(component, event, helper) {
                                           ...
                                           // console.log('items--->'+items[index].SOLIID);
                                            //console.log('Test Delete Action:-'+items[index].productName);
                                            //console.log('Test Or Items:-'+JSON.stringify(items));
                                            if(index>=0){	
                                               // console.log('items[index].SOLIID.length'+items[index].SOLIID.length);
                                                if(items[index].SOLIID!=null ){
                                                    //console.log('Test Sales Order Line Item;---->'+items[index].SOLIID);
                                                helper.deleteSaleOrderLineItem(component,event,items[index]);
                                                    items.splice(index, 1); 
                                                helper.normalUpdateRowCalculations(component, event, helper); 
                                            }else{
                                               // console.log('items--->else'+items);
                                                 items.splice(index, 1); 
                                               // console.log('start Delete not id:-->');
                                              //  helper.deleteOrderItem(component, event, index);
                                               // console.log('delete end');
                                                helper.normalUpdateRowCalculations(component, event, helper); //CR#174 -Chile Margin Block-SKI- kalpesh chande -  06/01/2023 
                                            } 
                                                 // items.splice(index, 1);
