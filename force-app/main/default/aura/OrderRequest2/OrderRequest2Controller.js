({
    ...
    onCheckchange:function(component, event, helper){
        var checkCmp = component.find("campChk");
        var isSimulated = component.get("v.isSimulated");
        var orderTypeList = component.get("v.orderTypeList");
        var opts=[];
        component.set("v.CampaignCheck",checkCmp.get("v.value"));
        component.set("v.newSalesOrder.Use_Campaign__c",checkCmp.get("v.value"));
        
        if(!checkCmp.get("v.value")){
        component.set("v.isStructure", checkCmp.get("v.value"));
        component.set("v.newSalesOrder.Campaign_Type__c","None");
         /* // For BONIFICAÇÃO removal.... 
            ...
            */
            ...
            component.set("v.isSimple", checkCmp.get("v.value"));
            ...
            //Order Type
                    opts.push({"class": "optionClass", label: $A.get("$Label.c.None"), value: 'None'});
                    for(var i=0; i< orderTypeList.length; i++){
                       if(orderTypeList[i] != 'BONIFICAÇÃO'){
                        //If New Simulation and Order Type=='Child' do not add order type in picklist
                        if(isSimulated && orderTypeList[i].search('ORDEM FILHA')==-1){
                            opts.push({"class": "optionClass", label: orderTypeList[i], value: orderTypeList[i]});
                        }
     ...
     onchageCampType : function(component, event) {
      ...
      var srcId = sourceRadio.getLocalId();
        if(srcId=='structCmp' && sourceRadio.get("v.value")){
         ...
         component.set("v.newSalesOrder.Campaign_Type__c","Structure");  
        }
    ...
    restrictQuantity: function(component, event, helper){
		...
		var setValue = null;
       // alert(JSON.stringify(target.get("v.labelClass")));
        ...
        if(qty){
            ...
            if(qtyString!=''){
                ...
                //#INC-407119 : Logic to covert decimal into integer then find Mod
                var NumOfZeroQ ;
                var NumOfZeroM;
               ...
               if(multipleOf && (qtyString%multipleOfString != 0 || qty<multipleOf)){
   ...
   validateInvoiceMessage  : function(component) {
    ...
    tabActionClicked: function(component, event, helper){
       ...
       // alert(selCurrncy);
        ...
        if(selCurrncy=='Billing BRL / Payment BRL'){
          ...
          //  alert(unitVal+' - '+minVal+' - '+monthlyInterestRate+' - '+fSPVal);
         }else
         if(selCurrncy=='Billing USD / Payment BRL' || selCurrncy=='Billing USD / Payment USD'){
        ...
        //updated By Prashant
           ...
           // alert(component.get("v.newSalesOrder.Sold_to_Party__c"));
        ...
        //updated By Prashant
                ...
                if(stateflag && validityflag){
                    ...
                    for (var idx=0; idx < orderItemList.length; idx++) {
                        if (idx==rowIndex) {
                           ...
                           // alert(itemRow.skuCategory);
                          
                            
                            //alert(orderItemList[idx].skuCategory);
                            ...
                            if(orderType=="ORDEM FILHA"){
                                ...
                                orderItemList[idx].minValue =  minVal;
                    ...
                    helper.updateRowCalculations(component, event);
                    component.set('v.orderItemList', orderItemList);
                    console.log('Line Item on Select -----> '+ JSON.stringify(component.get('v.orderItemList')));
                    //End function
            ...
            if(itemRow.Use_Campaign__c == true){
               console.log('itemRow.Use_Campaign__c'+itemRow.Use_Campaign__c);
            ...
            //  alert(itemRow.Price_Book__r.Interest_Date__c);
            component.set("v.blockDate",itemRow.Price_Book__r.Sales_Order_Block_Date__c);
               ...
               // alert(isChecked);
                component.find("Campaign").set("v.options", opts);
                 if(itemRow.Payment_Term__c != null){
                    var opts2=[];
                    opts2.push({"class": "optionClass", label: itemRow.ReloadPaymentTerms__c, value: itemRow.Payment_Term__c});
                    component.find("payTmdy").set("v.options", opts2);
                    component.set("v.newSalesOrder.Payment_Term__c", itemRow.Payment_Term__c); 
                    ...
                    var opts2=[];
                    opts2.push({"class": "optionClass", label: itemRow.Campaign_Payment_Term_Date__c, value: itemRow.Campaign_Payment_Term_Date__c});
                    component.find("payTmDt").set("v.options", opts2);
                    component.set("v.newSalesOrder.Campaign_Payment_Term_Date__c", itemRow.Campaign_Payment_Term_Date__c);
                   }
            }
            else{
             //  alert(isChecked);
     ...
     /* Change-Request Uncomment When needed*/
        ...
        for (var idx = 0; idx < orderItemList.length; idx++) {
            ...
            if(idx==rowIndex){
                 ...
                 //alert('Unit val '+orderItemList[idx].unitValue);
               ...
               // alert(value2);
                ...
                if(orderItemList[idx].timeInMonths && orderItemList[idx].interestRate!=0){
                    ...
                    console.log('presntValue1'+prsntValue);
                }
                else{
    ...
    onCampaignChange:function(component, event, helper){
        ...
        //alert(pb_id);        
           component.set("v.newSalesOrder.Price_Book__c",campaignId);
           helper.getPBPayMent(component, event, campaignId);
           helper.fetchPriceBookDetails(component);
        }
        else{
        helper.getpaymentTerm(component, event, helper,campaignId);
       }
    },
    ...
    handleCurrencyChange: function(component, event, helper){
        ...
        //if(event.getParam("oldValue")!=event.getParam("value")){
            ...
            if(isChecked){
                opts.push({"class": "optionClass", label: $A.get("$Label.c.Select_Campaign"), value: 'None'});
            } 
            else{
    ...
    handlePaymentTermChange : function(component, event){
        ...
        if(paymentTerm!='None'){
            ...
            //alert(pmObj.Maturity_Date_Mandatory__c);
            if(pmObj!=undefined){
    ...
    handlePaymentTermChangeDay : function(component, event){
       ...
       // alert(paymentTerm);
        ...
        if(paymentTerm!='None'){
            ...
            var maturityDate = component.get("v.newSalesOrder.Maturity_Date__c");
            console.log('PAyObj: '+JSON.stringify(pmObj));
            if(pmObj!=undefined){
                if(pmObj.Maturity_Date_Mandatory__c == $A.get("$Label.c.Yes") || pmObj.Maturity_Date_Mandatory__c == "Yes") {
                   // component.set("v.isMature", "true");
                }
        ...
        if(paymentTerm!='None'){
            ...
            if(pmObj!=undefined){
               ...
               // component.set("v.newSalesOrder.Maturity_Date__c", paymentTerm);
                ...
                component.set("v.newSalesOrder.Campaign_Payment_Term_Date__c", paymentTerm);
                component.set("v.newSalesOrder.Campaign_Payment_Term__c", pmObj.Id);
                //console.log('days: '+pmObj.Payment_Term__r.Days_to_calc_interest__c);
               // component.set("v.days", pmObj.Payment_Term__r.Days_to_calc_interest__c);
            }
